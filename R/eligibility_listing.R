...
eligibility_listing <- function(df, download = FALSE) {
  listing <- df %>%
    ...
    tidyr::separate(
      ...
      into = paste0(
        "PD Date",
        seq_len(max(sapply(strsplit(df$dvdtm, ";;;"), length)))
      ),
      sep = ";;;",
      fill = "right"
    ) %>%
    ...
    tidyr::separate(
      ...
      into = paste0(
        "PD Term",
        seq_len(max(sapply(strsplit(df$eligibility_criteria, ";;;"), length)))
      ),
      sep = ";;;",
      fill = "right"
    ) %>%
    ...
    arrange(.data$Site, .data$`Participant ID`)
    ...
    listing %>%
      ...
      text_transform(
        locations = cells_body(
          columns = c(`Which I/E`, starts_with("PD Term"))
        ),
        fn = function(x) {
          vapply(
            x,
...
#' @returns A `gt` object
#' @export
scrollable_gt <- function(
  gt_tbl,
  height = "300px",
  min_table_width = "1200px"
) {
  div(
    style = sprintf(
      "
