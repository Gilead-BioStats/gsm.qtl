...
if (rlang::is_installed("gsm.qtl")) {
  library(gsm.qtl)
}

devtools::load_all()

qualification_test_paths <- list.files(system.file("tests", "testqualification", "qualification", package = "gsm.qtl"), pattern = ".R", full.names = TRUE)


if (length(qualification_test_paths) > 0) {
  
  qualification_test_results <- lapply(qualification_test_paths, function(x) {
    test_results_raw <- testthat::test_file(x, reporter = testthat::ListReporter, package = "gsm.qtl")
