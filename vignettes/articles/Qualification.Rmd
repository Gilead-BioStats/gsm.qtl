...
if (rlang::is_installed("gsm.endpoints")) {
  library(gsm.endpoints)
}

devtools::load_all()

qualification_test_paths <- list.files(system.file("tests", "testqualification", "qualification", package = "gsm.endpoints"), pattern = ".R", full.names = TRUE)


if (length(qualification_test_paths) > 0) {
  
  qualification_test_results <- lapply(qualification_test_paths, function(x) {
    test_results_raw <- testthat::test_file(x, reporter = testthat::ListReporter, package = "gsm.endpoints")
    
    test_results_parsed <- data.frame(test_results_raw)
    
    output <- data.frame(
      File = test_results_parsed$file,
      Passed = !(test_results_parsed$failed | test_results_parsed$error),
