...
site_criteria_bar <- dfListing %>% 
  filter(!is.na(ietestcd_concat) | Source == "Ineligible -- PD only, check IE") %>% 
  mutate(
    ietestcd_concat = case_when(
      is.na(ietestcd_concat) ~ "Ineligible -- PD only, check IE",
...
country_criteria_bar <- dfListing %>% 
  ...
  mutate(ietestcd_concat = ifelse(is.na(ietestcd_concat), "Ineligible -- PD only, check IE", ietestcd_concat)) %>%  
...
dfListing %>% 
  ...
  gt::gt() %>%
  text_transform(
    locations = cells_body(columns = starts_with("PD Term")),
    fn = function(x) {
