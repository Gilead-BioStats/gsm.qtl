...
dfResults %>%
  ...
  mutate(Param = case_when(
    ...
    Param == "Numerator" ~ "Ineligible Participants",
...
site_bar <- dfListing %>% 
  ...
  mutate(invid = forcats::fct_rev(forcats::fct_infreq(invid))) %>%
  dplyr::group_by(invid, fillcol) %>%
  dplyr::summarize(totals = length(subjid)) %>%
  ungroup() %>%
  ggplot(., aes(y = invid,  fill = fillcol, x = totals,
  ...
  scale_fill_manual(values = c("Ineligible" = "#FF5859",
                               "No Eligiblity Risk" = "#00BFC4",
                               "Neither" = "#7CAE00")) +
  theme_classic() 
plotly::ggplotly(site_bar, tooltip = c("text")) %>%
  layout(margin = list(l = 50, r = 50, b = 150, t = 50),
         annotations = list(x = 1, y = -0.5, text = "Note: Excludes sites with no ineligible participants.",
...
country_bar <- dfListing %>% 
  ...
  mutate(country = forcats::fct_rev(forcats::fct_infreq(country))) %>%
  dplyr::group_by(country, fillcol) %>%
  dplyr::summarize(totals = length(subjid)) %>%
  ungroup() %>%
  ggplot(., aes(y = country, fill = fillcol, x = totals,
                text = paste0("Count: ", totals, 
                              "\nCountry: ",country, 
                             "\nEligibility Status: ", fillcol))) +
  geom_bar(stat = "identity") +
  labs(y = "Country", x = "Participant Count", fill = "Eligibility", title = "Count by country") +
  scale_fill_manual(values = c("Ineligible" = "#FF5859",
                               "No Eligiblity Risk" = "#00BFC4",
                               "Neither" = "#7CAE00")) +
  theme_classic() 
plotly::ggplotly(country_bar, tooltip = c("text")) %>%
  layout(margin = list(l = 50, r = 50, b = 150, t = 50),
         annotations = list(x = 1, y = -0.5, text = "Note: Excludes countries with no ineligible participants.",
                            xref='paper', yref='paper', showarrow = F, 
                            xanchor='right', yanchor='auto', xshift=0, yshift=0,
                            font = list(size = 10)))
```
...
site_criteria_bar <- dfListing %>% 
  ...
  ggplot(., aes(x = ietestcd_concat, fill = invid,
                text = paste0("Site: ",invid, 
                             "\nEligibility Status: ", ietestcd_concat))) +
  geom_bar() +
  labs(x = "Criteria", y = "Criteria Count", fill = "Site", title = "Eligibility by site") + 
  theme_classic(base_size = 11) + 
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),  # tilt to avoid overlap
    panel.grid.major.x = element_blank()
  ) 
plotly::ggplotly(site_criteria_bar, tooltip = c("y", "text")) %>%
  layout(margin = list(l = 50, r = 50, b = 150, t = 50),
...
country_criteria_bar <- dfListing %>% 
  ...
  ggplot(., aes(x = ietestcd_concat, fill = country,
                text = paste0("Country: ",country, 
                             "\nEligibility Status: ", ietestcd_concat))) +
  geom_bar() +
  labs(x = "Criteria", y = "Criteria Count", fill = "Country", title = "Eligibility by Country") + 
  theme_classic(base_size = 11) + 
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),  # tilt to avoid overlap
    panel.grid.major.x = element_blank()
  ) 
plotly::ggplotly(country_criteria_bar, tooltip = c("y", "text")) %>%
  layout(margin = list(l = 50, r = 50, b = 150, t = 50),
         annotations = list(x = 1, y = -0.5, text = "Note: Participants can be ineligible for multiple criteria.",
                            xref='paper', yref='paper', showarrow = F, 
                            xanchor='right', yanchor='auto', xshift=0, yshift=0,
